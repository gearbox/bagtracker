...
Revision ID: e5cdac811933
Revises:
Create Date: 2025-09-29 02:26:51.201257
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("created_by", sa.UUID(), nullable=True),
        sa.Column("updated_by", sa.UUID(), nullable=True),
        sa.CheckConstraint("current_price_usd >= 0", name="non_negative_price"),
        sa.CheckConstraint("decimals >= 0 AND decimals <= 77", name="valid_decimals"),
    ...
    op.create_index("ix_balance_history_date_wallet", "balances_history", ["snapshot_date", "wallet_id"], unique=False)
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("cex_account_id", sa.UUID(), nullable=True),
        ...
        sa.ForeignKeyConstraint(["cex_account_id"], ["cex_accounts.uuid"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["chain_id"],
            ["chains.id"],
        ),
        ...
        sa.ForeignKeyConstraint(["wallet_id"], ["wallets.uuid"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("uuid"),
    )
    op.create_index("idx_tx_uuid", "transactions", ["uuid"], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index("idx_tx_uuid", table_name="transactions")
    ...
    op.drop_index("ix_balance_history_date_wallet", table_name="balances_history")
